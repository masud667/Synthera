...
export default function WishlistItems() {
  ...
  const { data: wishlistItems = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      const res = await axios.get(`/api/wishlist?email=${session?.user?.email}`);
  ...
  const removeMutation = useMutation({
    mutationFn: (id) => axios.delete(`/api/wishlist/${id}`),
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ["wishlist", session?.user?.email] }),
  });

  // Move item to cart (using your addToCart hook)
  const handleMoveToCart = async (item) => {
    await addToCart(item, item.size || "M", item.quantity || 1);
    removeMutation.mutate(item._id); // remove from wishlist after adding to cart
  };
    ...
    >
      ...
      >
        <span className="font-semibold text-gray-300">
          Total Items: <span className="text-yellow-500">{wishlistItems.length}</span>
        </span>
        <span className="text-lg font-bold text-gray-200">
          Total: <span className="text-yellow-500">
